...
2. このパネルには以下の要素を追加します：
   ...
   - 「閉じる」ボタン
   - 確認用の「はい、終了します」ボタン（名前は「ConfirmButton」に設定）
3. `UIController` スクリプトの対応するプロパティにこれらの要素も接続します

### 7. 終了ボタンの追加

1. `BottomPanel` に新しい `Button` を追加し、`ExitButton` という名前を付けます
2. ボタンのテキストを「終了」に設定します
3. ボタンの色を赤色などの目立つ色に設定します（警告色として）
4. ボタンの位置を調整し、他のコントロールから十分に離して誤クリックを防ぎます
5. `UIController` スクリプトの exitButton フィールドにこのボタンをインスペクタでドラッグして設定します
...
標準のUIControllerとは別に、ステップバイステップで進行するウィザード形式のUIも実装されています。こちらはよりシンプルな操作体験を提供します。

### UIWizardプレハブの利用方法

1. `Assets/Prefabs/WizardUI.prefab` をシーンにドラッグ＆ドロップします
2. 必要なコンポーネントが自動的に配置されます
3. プレイモードで動作を確認します

### UIWizardの主な特徴

- ステップバイステップでの誘導UI（タイトル→ISO選択→USB選択→確認→書き込み→完了）
- 各ステップ間を行き来できる「戻る」「次へ」ボタン
- 分かりやすいアイコンとメッセージによるフィードバック
- モバイルフレンドリーなインターフェース設計

### ウィザードUIのカスタマイズ

UIWizardコンポーネントでは以下の要素をInspectorから設定できます：

1. **各ステップのパネル**: 
   - TitlePanel, ISOSelectionPanel, USBSelectionPanel など各ステップのUI
   - それぞれにデザインやレイアウトをカスタマイズ可能

2. **リスト表示部分**:
   - ISOリストやUSBデバイスリストのコンテンツ領域とプレハブ
   - 選択状態表示用のテキスト

3. **確認画面の要素**:
   - ISO情報、デバイス情報、警告メッセージのテキストフィールド

4. **進捗表示部分**:
   - プログレスバー、進捗パーセンテージ、状態メッセージ

5. **完了画面の要素**:
   - 成功/エラーアイコン、完了メッセージ、終了ボタン

6. **ナビゲーションボタン**:
   - 戻る・次へ・書き込み・タイトルへ戻る

### UIWizardとUIControllerの切り替え

両方のUIをシーンに配置することも可能です。その場合、不要なUIを最初に非表示にしてください：

```csharp
// 標準UIを非表示にする例
GameObject.FindWithTag("StandardUI").SetActive(false);

// ウィザードUIを非表示にする例
GameObject.FindWithTag("WizardUI").SetActive(false);
```

UI切り替えボタンを用意すれば、ユーザーが好みのインターフェースを選択できるようになります。
