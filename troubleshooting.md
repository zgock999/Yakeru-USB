...
- デバイスのキャッシュやバッファリングによる見かけ上の高速書き込み

**解決方法**:

1. **アプリケーションの最新バージョンの使用**:
   - 最新版では進捗計算のアルゴリズムが改善されています

2. **デバイスの準備**:
   - 書き込み前に `force_clean_disk.bat`（Windows）を使用してディスクを完全にクリーンアップ
   - Linux/macOSでは事前に `sudo dd if=/dev/zero of=/dev/sdX bs=4M count=10` などでディスクの先頭部分をクリア

3. **大きなISOファイルでテスト**:
   - 小さなISOファイルは瞬時に書き込まれるため進捗表示が不正確になることがある
   - 1GB以上のISOファイルで進捗表示をテスト

4. **バッファフラッシュの待機**:
   - 書き込み完了後も実際のディスクへの書き込みが完了するまで待機が必要な場合がある
   - フラッシュ中は99%で停滞することがありますが、これは正常な動作です

5. **USB接続を確認**:
   - USB 3.0デバイスをUSB 2.0ポートに接続すると転送速度が低下し、進捗表示がスムーズになる
   - USB接続の品質が不安定な場合、進捗報告が不規則になることがある
